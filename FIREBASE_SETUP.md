...
1. เปิดแอพ → ที่หน้า Entry Gate กด **ล็อกอินครู** → ล็อกอินด้วย Google
   (ครั้งแรกจะขึ้นข้อความ "บัญชี ... ยังไม่ได้รับสิทธิ์ครู" เพราะ uid ยังไม่อยู่ใน allowlist — ไม่เป็นไร ทำต่อขั้น 2)
...
4. รีเฟรชแอพ → กด "ล็อกอินครู" ที่ Gate อีกครั้ง → แอพจะ dismiss Gate ทันที (ไม่มี popup ซ้ำเพราะ Firebase session ยังคาอยู่) → ปุ่ม **Editor**, **บันทึกขึ้น Cloud**, และ **Practice History** จะปรากฏ

หมายเหตุ: ผล `isAdmin` ถูก cache ใน sessionStorage 24 ชั่วโมงเพื่อลดเวลาโหลด — ถ้าเพิ่งเพิ่ม uid ใน `admins` แล้วยังไม่เห็นสิทธิ์ครู ให้ปิดแท็บแล้วเปิดใหม่ (`signOut` ก็ล้าง cache ให้เช่นกัน)

## การใช้งานหลัง setup

- **นักเรียน** — ที่ Entry Gate กด **เข้าใช้งาน** เข้าไปได้เลย ไม่ต้องล็อกอิน (อ่านอย่างเดียว) — การเล่นเพลงจะถูกบันทึกลง Practice Log แบบไม่ระบุตัวตนโดยอัตโนมัติ
- **ครู** — ที่ Gate กด **ล็อกอินครู** → เปิด Editor แก้เนื้อร้อง/คอร์ด/โน้ต+Tab → กด **บันทึกขึ้น Cloud** ข้อมูลเข้า Firestore ทันที
- **Practice History** — ปุ่มมองเห็นได้เฉพาะครูที่ล็อกอินแล้ว เป็น log รวมของทุกคน/ทุกเครื่อง (ไม่แยกรายนักเรียน) เก็บข้อมูลย้อนหลัง 90 วัน แล้วลบข้อมูลเก่าให้อัตโนมัติทุกครั้งที่ครูเปิดหน้านี้ — ถ้าข้อมูลพุ่งเกิน 2,000 รายการ (ผิดปกติ) ระบบจะล้างทั้งหมดทันทีแทนการทยอยลบ ปรับค่าได้ที่ `RETENTION_DAYS`/`MAX_DOCS` ใน [src/firebase/practiceLogStore.js](src/firebase/practiceLogStore.js)
- **Performance** — แอพ cache ข้อมูลเพลง 3 ชั้น (in-memory → sessionStorage → Firestore) และโหลด MP3 คู่ขนานกับ Firestore fetch (ประหยัด ~300-800ms/เพลงบนมือถือ) รายละเอียดใน `CLAUDE.md`
