...
**Pitch picker modal (`#ntPitchModal` / `.ntpitch-overlay`):** `z-index: 460`. Replaces the old free-text pitch input — each notation row shows a `.editor-pitch-btn` button displaying the current pitch; clicking opens this modal. Grid layout: header row of octave numbers (3 / 4 / 5) + 12 note rows (C → B, sharps muted visually) + a "rest (หยุด)" row. Single tap selects the pitch, updates the row via `updateNoteField()`, re-renders, and closes. Currently selected pitch is highlighted (`.is-selected`). Adding a new row via the insert divider (`makeNotationInsertDivider`) opens this modal automatically. Wired inside `wireNotationConfigControls()` via `buildNtPitchGrid()`. Escape / backdrop closes without applying. Bottom sheet on mobile, centered dialog on desktop.
